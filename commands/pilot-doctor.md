...
2. **Hook scripts executable + present** — run:
   ...
   for h in plan-gate.sh pre-commit.sh safety-gate.sh verify-gate.sh capture-test-run.sh sessionstart-banner.sh precompact-anchor.sh route-advisor.sh log-skill-invocation.sh; do
...
3. **settings.json wiring** — list each wired pilot hook AND verify the
   ...
   ' "$HOME/.claude/settings.json" 2>/dev/null \
     | grep -E '/hooks/(plan-gate|pre-commit|safety-gate|verify-gate|capture-test-run|sessionstart-banner|precompact-anchor|route-advisor|log-skill-invocation)\.sh' \
   ...
   jq -e '.hooks // {} | [.. | objects | .command? // empty] | any(test("/hooks/(plan-gate|pre-commit|safety-gate|verify-gate|capture-test-run|sessionstart-banner|precompact-anchor|route-advisor|log-skill-invocation)\\.sh"))' \
